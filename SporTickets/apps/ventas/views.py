...
from apps.ventas.forms import SeleccionarEventoForm, CantidadForm
...
def mis_compras(request):
    compras = request.user.compras_del_usuario.all()
...
def comprar_boletos(request, id_evento):
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            venta = Venta.obtener_venta_activa(request, request.user, "nombre_vendedor")
            indice = 0
            for form in formset:
                print(form.as_table())
